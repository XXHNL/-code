...
void OLED_ShowString(u8 x,u8 y,u8 *chr,u8 size1,u8 mode)
{
	while((*chr>=' ')&&(*chr<='~'))
	{
		OLED_ShowChar(x,y,*chr,size1,mode); //  在OLED屏幕上显示字符
		if(size1==8)x+=6; //  如果字符大小为8，则x坐标加6
		else x+=size1/2; //  否则x坐标加字符大小的一半
		chr++; //  字符指针加1，指向下一个字符
  }
}
...
void OLED_Init(void)
{
  ...
  GPIO_InitTypeDef  GPIO_InitStructure;
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_GPIOB,ENABLE);//???PORTA~E,PORTG???
