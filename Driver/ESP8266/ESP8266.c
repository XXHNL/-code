...
// ESP8266 接收数据函数示例
// 假设串口接收缓冲区为 RX_Buffer，长度为 RX_MAX_LENGTH

// 返回接收到的数据指针，未收到数据返回 NULL
// unsigned char* ESP8266_GetIPD(uint16_t timeout_ms)
// {
//     static unsigned char ipd_buf[RX1_MAX_LENGTH] = {0};
//     uint16_t len = 0;
//     uint32_t tickstart = get_tick(); // 需要你实现获取当前tick的函数
...
//         {
//             RX1_Compelete = 0;
//             // 假设 Usart1RecBuf 是你的串口1接收缓冲区
//             memcpy(ipd_buf, Usart1RecBuf, RxCounter);
//             ipd_buf[RxCounter] = 0; // 结尾补0
//             len = RxCounter;
//             RxCounter = 0;
//             return ipd_buf;
//         }
